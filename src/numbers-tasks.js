...
function roundToPowerOfTen(num, pow) {
  const innerPow = 10 ** pow;
  return Math.round(num / innerPow) * innerPow;
}
...
function getSumOfDigits(num) {
  ...
  while (subValue / 10 > 0) {
    sumNum += subValue % 10;
    subValue = Math.trunc(subValue / 10);
  }
  return sumNum;
}
